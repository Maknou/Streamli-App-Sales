...
## drop any na left in the data
    ...
    genre = st.radio('Sections',
        ('Main','Sales', 'Orders', 'Products','Delivery','Delivey Risk ML'))
    ...
    if genre == 'Main':
        ...
        data =data1[['Transaction Type', 'Customer Segment','Department Name', 'Order Region','Order Status','Shipping Mode']]

        y = data1['Late delivery risk']
        X = data

        x_train, x_test, y_train, y_test = train_test_split(X,y, test_size = 0.3, random_state = 100)

        x_train['Transaction Type'] = x_train['Transaction Type'].cat.codes
        x_test['Transaction Type'] = x_test['Transaction Type'].cat.codes

        x_train['Department Name'] = x_train['Department Name'].cat.codes
        x_test['Department Name'] = x_test['Department Name'].cat.codes

        x_train['Customer Segment'] = x_train['Customer Segment'].cat.codes
        x_test['Customer Segment'] = x_test['Customer Segment'].cat.codes

        x_train['Order Region'] = x_train['Order Region'].cat.codes
        x_test['Order Region'] = x_test['Order Region'].cat.codes

        x_train['Order Status'] = x_train['Order Status'].cat.codes
        x_test['Order Status'] = x_test['Order Status'].cat.codes

        x_train['Shipping Mode'] = x_train['Shipping Mode'].cat.codes
        x_test['Shipping Mode'] = x_test['Shipping Mode'].cat.codes

        model = RandomForestClassifier(max_depth=8, random_state = 10)
        ...
        def prediction(transaction,region,segment,department,status,shipping):
            ...
            if region == "Southeast Asia":
                ...
                region = 5
            elif region == "West of USA":
                region = 6
            elif region == "US Center":
                region = 7
            ...
            if department == "Fitness":
                ...
                department = 1
            elif department == "Golf":
                department = 2
            elif department == "Footwear":
                department = 3
            elif department == "Outdoors":
                department = 4
            elif department == "Fan Shop":
                department = 5
            elif department == "Technology":
                department = 6
            elif department == "Book Shop":
                department = 7
            elif department == "Discs Shop":
                department = 8
            elif department == "Pet Shop":
                department = 9
            else:
                department = 10
            ...
            if shipping == "First Class":
                shipping = 0
            elif shipping == "Same Day":
                shipping = 1
            elif shipping == "Second Class":
                shipping = 2
            else:
                shipping = 3

            features = np.array([transaction,region,segment,department,status,shipping])
        ...
        def main():
            ...
            <h1 style ="color:black;text-align:center;">Delivery Risk App</h1>
            </div>
            """

            st.markdown(html_temp, unsafe_allow_html = True)

            transactions = data1['Transaction Type'].drop_duplicates()
            regions = data1['Order Region'].drop_duplicates()
            departments = data1['Department Name'].drop_duplicates()
            segments = data1['Customer Segment'].drop_duplicates()
            statuses = data1['Order Status'].drop_duplicates()
            shipments = data1['Shipping Mode'].drop_duplicates()

            transaction = st.selectbox('Select Transaction mode',transactions)
            region= st.selectbox('Select Order Region',regions)
            department= st.selectbox('Select Purchasing Department',departments)
            segment= st.selectbox('Select Customer Segment',segments)
            status= st.selectbox('Select Order Status',statuses)
            shipping= st.selectbox('Select Shipping mode',shipments)

            result = ""

            if st.button('Predict'):
                result = prediction(transaction,region,segment,department,status,shipping)
                st.success('Your Order has a delivery risk labeled {}'.format(result))
